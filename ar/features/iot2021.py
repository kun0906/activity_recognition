...
from itod.pparser.pcap import PCAP, pcap2flows, flow2subflows
from itod.utils.tool import dump_data, load_data
from reprst.report.paper_results import check_path
	...
	if not os.path.exists(device_meta_file):
		...
		for i, (activity_name, files) in enumerate(device_files.items()):
			...
			for j, f in enumerate(files):
				...
				pp.keep_ip(f, kept_ips=[device2mac[device]], output_file=filtered_f)  # only srcIP
