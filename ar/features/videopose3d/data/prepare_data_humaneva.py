...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--path', default='', type=str, metavar='PATH',
                        help='path to the processed HumanEva dataset')
    parser.add_argument('--convert-3d', action='store_true', help='convert 3D mocap data')
    parser.add_argument('--convert-2d', default='', type=str, metavar='PATH',
                        help='convert user-supplied 2D detections')
                ...
                for i, chunk in enumerate(all_chunks):
                    ...
                    if np.isfinite(chunk).all():
                        ...
                        output_2d[subject][name] = list(
                            np.array(all_chunks_2d[i], dtype='float32').transpose(1, 0, 2, 3))
        ...
        for f in file_list:
            ...
            if subject in frame_mapping and action in frame_mapping[subject]:
                ...
                for (start_idx, end_idx, labeled, split, name) in chunks:
                    ...
                    assert len(kps) == end_idx - start_idx, "Got len {}, expected {}".format(len(kps),
                                                                                             end_idx - start_idx)
