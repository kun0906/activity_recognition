...
class L2Part(nn.Module):
    ...
    def __init__(self, in_channels, stage_out_channels):
        ...
        self.secondStage = stage(1, in_channels + stage_out_channels, in_channels, in_channels * 4, stage_out_channels,
                                 'L2')
        self.thirdStage = stage(2, in_channels + stage_out_channels, in_channels, in_channels * 4, stage_out_channels,
                                'L2')
        self.fourthStage = stage(3, in_channels + stage_out_channels, in_channels, in_channels * 4, stage_out_channels,
                                 'L2')
...
class stage(nn.Module):
    ...
    def __init__(self, stageID, in_channels, out_channels_perSub, mid_channels, out_channels, appendix):
        ...
        self.afterConcatsFirst = nn.Sequential(OrderedDict(
            {'Mconv6_stage%d_%s' % (stageID, appendix): conv2d, 'Mprelu6_stage%d_%s' % (stageID, appendix): prelu}))
...
class concatLayer(nn.Module):
    ...
    def __init__(self, in_channels, out_channels_perSub, i, j, appendix):
        ...
        self.secondSub = self.concatLayerSub(out_channels_perSub, out_channels_perSub,
                                             '%d_stage%d_' % (i, j) + appendix + '_1')
        self.thirdSub = self.concatLayerSub(out_channels_perSub, out_channels_perSub,
                                            '%d_stage%d_' % (i, j) + appendix + '_2')
...
def openPose(pretrained=True, **kwargs):
    ...
    if pretrained:
        ...
        pretrained_dict = {l: torch.from_numpy(numpy.array(v)).view_as(p) for k, v in state_dict.items() for l, p in
