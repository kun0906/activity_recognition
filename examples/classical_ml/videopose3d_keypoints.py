...
def split_train_test_video(meta, video_type='_1.mp4', test_size=0.3, random_state=42):
    ...
    #     feat = feat.replace('_1_mp4.npy', '_1-mirrored_mp4.npy')
    ...
    #     f = f.replace('_1.mp4', '_1-mirrored.mp4')
    #     feat = feat.replace('_1.mp4.npy', '_1-mirrored_mp4.npy')
    #     if not os.path.exists(f) or not os.path.exists(feat):
...
def parse_logs(in_dir='data/data-clean/log'):
    root_dir = 'examples/data/data-clean'
        ...
        for i, vs in enumerate(train):
            ...
            if not is_test:
                if feature_type == 'mean':
                    ...
                    x = extract_feature_fixed_segments(f, dim=50)
            elif is_test:
                if feature_type == 'mean':
                    x = extract_feature_average(f)
                elif feature_type == 'sampling':
...
def gen_Xy(in_dir, out_dir, is_subclip=True, is_mirror=True, is_cnn_feature=True, device_type='refrigerator'):
    ...
    issued_videos = pd.read_csv(os.path.join('examples/data/data-clean/refrigerator', 'issued_videos.csv'),
                                header=None).values[
                    :, -1].tolist()
                ...
                for f in sorted(os.listdir(participant_dir)):
                    ...
                    try:
                        ...
                        if is_subclip:
                            ...
                            x_feat, kept_durations = _extract_3d_keypoint_feature(x, out_dir=out_dir_tmp,
                                                                                  is_trimming=False)
                            # kept_durations: number of frames kept
                            if kept_durations == 0: continue
                            if '1.mp4.npy' in x_feat:
                                durations['camera1'].append((kept_durations, 1, ''))
                                if kept_durations > 548:  # 600s
                                    print(
                                        f'---file_path: {x_feat}, kept_durations: {kept_durations}, is_trimming: False')
                                    long_video_cnt += 1
                            elif '2.mkv.npy' in x:
                        ...
                        raise ValueError(msg)
                    i += 1
    print(f'long_video_cnt: {long_video_cnt}')
    for key, vs in durations.items():
        ...
        fps = fps[0]
        qs = [0, 0.25, 0.5, 0.75, 0.9, 0.95, 0.96, 0.975, 0.98, 0.99, 1.0]
...
label2color = {'no_interaction': 'r', 'open_close_fridge': 'g', 'put_back_item': 'b', 'screen_interaction': 'c',
               'take_out_item': 'k'}
...
def main():
    ...
    # Videopose3d keypoints as input_data
    in_dir = f'examples/out/keypoints3d/data/data-clean/refrigerator'
    if not os.path.exists(in_dir):
        os.makedirs(IN_DIR)
    out_dir = f'examples/out/keypoints3d-trimmed/data/data-clean/refrigerator'
    Xy_train_test_file = f'{out_dir}/Xy_train_test_3d_keypoints.dat'
    ...
    if not os.path.exists(Xy_train_test_file):
        ...
        Xy_cnn_features_file = f'{in_dir}/Xy_3d_keypoints_features.dat'
        ...
        if not os.path.exists(Xy_cnn_features_file):
            ...
            in_raw_dir = 'examples/out/keypoints3d/data/data-clean/refrigerator'
            ...
            meta = gen_Xy([in_raw_dir], out_dir=out_dir, is_subclip=True, is_mirror=False, is_cnn_feature=False)
    ...
    # X_test_meta, X_test, y_test = augment_test(test_meta, augment_type='camera_1',
    #                                            is_mirror=False)
    ...
    # plot_3dkeypoints(X_train, y_train, random_state=RANDOM_STATE, title='Train set')
    # return # debug
    ...
    res = []
    for model_name in ['OvRLogReg', 'RF', 'SVM(linear)']:  # ['OvRLogReg', 'SVM(linear)', 'RF']
        print(f'\n\n***{model_name}')
        start_time = time.time()
        ...
        # # 3.2 evaluate on the train set
        # y_train_preds = detector.model.predict(X_train)
        # print('y_train (label, cnt): ', sorted(Counter(y_train).items(), key=lambda x: x[0]))
        # acc = sklearn.metrics.accuracy_score(y_train, y_train_preds)
        # print(f'train accuracy: {acc}')
        # # res.append((acc, n_estimators))
        # cm = sklearn.metrics.confusion_matrix(y_train, y_train_preds)
        # print(cm)
        # print('\nTesting')
        ...
        err_mp = {}
        misclassified_dir = f'{out_dir}/misclassified'
        _tmp_dir = os.path.join(misclassified_dir, model_name)
        if os.path.exists(_tmp_dir):
            shutil.rmtree(_tmp_dir)
        X_misclassified = []
        y_misclassified = []
        start_dir = 'examples/out/keypoints3d/'
        for x_test_file_, x_test_, y_t, y_p in zip(X_test_meta, X_test, y_test, y_preds):
            if y_t != y_p:
                name = f'{idx2label[y_t]}-{y_t}:{x_test_file_}'
                ...
                out_file_ = os.path.join(tmp_out_dir_, os.path.relpath(x_test_file_, start_dir))
                tmp_out_dir = os.path.dirname(out_file_)
                if not os.path.exists(tmp_out_dir):
                    os.makedirs(tmp_out_dir)
                print(x_test_file_, y_t, y_p)
                copyfile(x_test_file_, out_file_)  # copy 3d keypoints file
                x_test_video_file_ = os.path.join('examples', os.path.relpath(x_test_file_, f'{start_dir}'))[
                                     :-4]  # [:-3] remove '.npy'
                out_video_file_ = os.path.join(tmp_out_dir_, os.path.relpath(x_test_video_file_, f'examples'))
                try:
                    copyfile(x_test_video_file_, out_video_file_)  # copy video file
                except Exception as e:
                    print(f'Error: e')
                # print(f'{mp[y_t]} -> {mp[y_p]}')
                X_misclassified.append(x_test_file_)
                y_misclassified.append(y_t)

        # plot_3dkeypoints(np.asarray(X_misclassified), np.asarray(y_misclassified).reshape(-1, ),
        #                  random_state=RANDOM_STATE, title='Test set (misclassified)')
