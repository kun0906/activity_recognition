...
def get_fft_features(raw_data='', m=84, keypoint=7, file=''):
	...
	#
	# if flg == 'entire':
	# 	# res = list(np.mean(raw_data, axis=0))
	# 	# res = list(np.std(raw_data, axis=0))
	# 	# res = list(np.mean(raw_data, axis=0)) + list(np.std(raw_data, axis=0))
	# 	res = np.mean(raw_data, axis=0).tolist() + np.std(raw_data, axis=0).tolist() + skew(raw_data, axis=0).tolist() + \
	# 	      kurtosis(raw_data, axis=0).tolist() + np.min(raw_data, axis=0).tolist() + np.max(raw_data, axis=0).tolist()
	#
	# 	# res = np.std(raw_data, axis=0).tolist() + skew(raw_data, axis=0).tolist()
	# 	# [np.mean(data), np.std(data), skew(data), kurtosis(data), np.min(data), np.max(data)]
	# 	# A[np.isnan(A)] = 0
...
def _main(m=10, random_state=10):
	...
	for n_comp in n_comps:
		...
		# 	cm = confusion_matrix(test_y, pred_y)
		# 	print(cm)
		# 	cm_file = make_confusion_matrix(cm, categories=label2idx.keys(), title=f'SVM-linear-{idx2camera[data_type]}', out_dir='')
		# 	train_acc = accuracy_score(train_y, svm.predict(train_x))
		# 	acc = accuracy_score(test_y, pred_y)
		# 	print(acc, train_acc)
		# 	# missclassified = get_missclassified(cm, test_y, pred_y, test_raw_files, model_name = 'RF', idx2label=idx2label)
