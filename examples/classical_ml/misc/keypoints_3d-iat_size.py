...
def split_train_test_video(video_meta, traffic_meta, feature_type, video_type='_1.mp4', test_size=0.3, random_state=42):
    ...
    data = []
    meta = {'data': data}
    for (_, keypoint_3d_file, y) in video:
        if '-mirrored.mp4' in keypoint_3d_file: continue

        if '_1.mp4.npy' in keypoint_3d_file:
            traffic_feat = keypoint_3d_file.replace('_1.mp4.npy', f'.pcap-{feature_type}.dat')
        elif '_2.mkv.npy' in keypoint_3d_file:
            traffic_feat = keypoint_3d_file.replace('_2.mkv.npy', f'.pcap-{feature_type}.dat')
        elif '_3.mp4.npy' in keypoint_3d_file:
            traffic_feat = keypoint_3d_file.replace('_3.mp4.npy', f'.pcap-{feature_type}.dat')
        else:
            continue
        data.append((keypoint_3d_file, traffic_feat, y))  # f1: video_feature, f2: traffic_features
...
def aggregate_label(y):
    ...
    return np.asarray(new_y).reshape(-1, )
...
def main(random_state=42, root_dir='out3d_pred'):
    ...
    Xy_train_test_file = f'{in_dir}/Xy_train_test_3d_keypoints+iat_size.dat'
    ...
    is_agg = False
    if is_agg:
        label2idx = {'no_interaction': 0, 'open_put_take': 1, 'screen_interaction': 2}
        idx2label = {0: 'no_interaction', 1: 'open_put_take', 2: 'screen_interaction'}
        y_train = aggregate_label(y_train)
        y_test = aggregate_label(y_test)
