    ...
    def __init__(self, inputs, filters, is_training, strides,
        ...
        if use_projection:
            ...
            self.shortcut = nn.Sequential(SamePadding((1, 1, 1), (1, strides, strides)),
                                          nn.Conv3d(inputs, filters_out, kernel_size=1, stride=(1, strides, strides),
                                                    bias=False, padding=0),
                                          nn.BatchNorm3d(filters_out),
                                          nn.BatchNorm3d(
                                              filters_out))  # there are two, due to old models having it. To load weights, 2 batch norms are needed here...

        self.layers = nn.Sequential(SamePadding((T, 1, 1), (1, 1, 1)),
                                    nn.Conv3d(inputs, filters, kernel_size=(T, 1, 1), stride=1, padding=(0, 0, 0),
                                              bias=False),
                                    # 1
                                    nn.BatchNorm3d(filters),  # 2
                                    nn.ReLU(),
                                    SamePadding((1, 3, 3), (1, strides, strides)),
                                    nn.Conv3d(filters, filters, kernel_size=(1, 3, 3), stride=(1, strides, strides),
                                              bias=False, padding=0),
                                    # 5
                                    nn.BatchNorm3d(filters),  # 6
                                    nn.ReLU(),
                                    nn.Conv3d(filters, 4 * filters, kernel_size=1, stride=1, bias=False, padding=0),
                                    # 8
...
def resnet_3d_v1(resnet_depth, num_classes, data_format='channels_last', is_3d=True, non_local=[0, 0, 0, 0],
                 rep_flow=[0, 0, 0, 0, 0]):
