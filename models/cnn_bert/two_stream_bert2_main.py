...
from models.cnn_bert import video_transforms
from models.cnn_bert import datasets
from models.cnn_bert.opt.AdamW import AdamW
from models.cnn_bert.utils.model_path import rgb_3d_model_path_selection
from models.cnn_bert.datasets.extract_frames import extract_frames_main
import models.cnn_bert.all_models as MODELS
...
def build_model():
    ...
    if modality == "rgb":
        model_path = rgb_3d_model_path_selection(root_dir, args.arch)
        ...
        if "3D" in args.arch:
            if 'I3D' in args.arch:
                ...
                model_path = f'{root_dir}/weights/Flow_Kinetics_64f.pth'
                # model_path = os.path.join(modelLocation,'model_best.pth.tar')
    elif modality == "both":
        model_path = ''
    else:
        raise NotImplementedError(f'Not Implemented Error: {args.arch}')
    ...
    elif args.dataset == 'refrigerator':
        print('model path is: %s' % (model_path))
        model = MODELS.__dict__[args.arch](modelPath=model_path, num_classes=5, length=args.num_seg)
    else:
        raise NotImplementedError(f'Not Implemented Error: {args.dataset}')
...
def train(train_loader, model, criterion, criterion2, optimizer, epoch, modality):
    ...
    for i, (inputs, targets) in enumerate(train_loader):
        print(f'******i:{i + 1}th/{len(train_loader)} batch, half: {HALF}')
        # if inputs.shape != targets.shape: continue
        ...
        if i == 0:
            cm = confusion_matrix(targets.data, output.data.argmax(dim=1))
        else:
            cm += confusion_matrix(targets.data, output.data.argmax(dim=1))
        prec1, prec3 = accuracy(output.data, targets, topk=(1, 1))
...
def validate(val_loader, model, criterion, criterion2, modality, val_type='val'):
    batch_time = AverageMeter()
    lossesClassification = AverageMeter()
    top1 = AverageMeter()
    top3 = AverageMeter()
    ...
    with torch.no_grad():
        ...
        print(f'confusion matrix on {val_type} set:')
        print(cm)
...
def check_data(msg):
    in_dir = 'datasets/hmdb51_frames'
    if not os.path.exists(in_dir):
        raise ValueError(msg)


def _save_data(X, Y, out_file):
    # save data to disk
    with open(out_file, 'w') as out:
        for x, y in zip(X, Y):
            out.write(f'{x} None {y}\n')


def generate_train_test(in_dir, video_type='_1.mp4', test_size=0.3, random_state=42, modality=1, args=None):
    X = []
    Y = []

    for activity in sorted(os.listdir(in_dir)):
        if '.DS_Store' in activity: continue
        activity_dir = os.path.join(in_dir, activity)
        for part_id in sorted(os.listdir(activity_dir)):
            ...
            for f in sorted(os.listdir(part_id_dir)):
                if '.DS_Store' in f: continue
                f_dir = os.path.join(part_id_dir, f)
                if video_type == '_1.mp4' and '_1.mp4' in f:
                    X.append(f_dir)
                    Y.append(str(label2idx[activity]) + f' {activity}')
                elif video_type == '_3 2.mp4' and '_1.mp4' in f:
                    pass
                else:
                    # raise NotImplementedError(f'{video_type} NotImplementedError')
                    continue

    X_train, X_test, y_train, y_test = train_test_split(X, Y, test_size=test_size,
                                                        random_state=random_state, shuffle=True)

    print(f'Xy_train~: {len(X_train)}')
    print(f'Xy_test: {len(X_test)}')

    # save data to disk
    train_setting_file = "train_%s_split%d.txt" % (modality, args.split)
    train_split_file = os.path.join(args.settings, args.dataset, train_setting_file)
    _save_data(X_train, y_train, out_file=train_split_file)

    test_setting_file = "test_%s_split%d.txt" % (modality, args.split)
    test_split_file = os.path.join(args.settings, args.dataset, test_setting_file)
    _save_data(X_test, y_test, out_file=test_split_file)

    print(train_split_file, test_split_file)
    return train_split_file, test_split_file


def generate_train_val(train_split_file, val_size=0.1, random_state=42, modality=1, args=None):
    with open(train_split_file, 'r') as f:
        data = f.readlines()

    Xy_train, Xy_val = train_test_split(data, test_size=val_size,
                                        random_state=random_state, shuffle=True)

    print(f'Xy_train: {len(Xy_train)}')
    print(f'Xy_val: {len(Xy_val)}')
    # save data to disk
    train_setting_file = "train_%s_split%d.txt" % (modality, args.split)
    train_split_file = os.path.join(args.settings, args.dataset, train_setting_file)
    ...
    print(train_split_file, val_split_file)
    return train_split_file, val_split_file
...
def main():
    ...
    print(f'input_size: {input_size}, width: {width}, height: {height}')

    # save results
    saveLocation = f"{root_dir}/checkpoint/" + args.dataset + "_" + args.arch + "_split" + str(args.split)
    ...
    ##################################################################################################################
    # step 2.3  define loss function (criterion) and optimizer
    ...
    if args.dataset == 'ucf101':
        ...
        # step 2.3 generate frames from videos
        dataset = 'models/cnn_bert/datasets/refrigerator_frames'
        extract_frames_main(in_dir='data/data-clean/refrigerator', out_dir=dataset, overwrite=False)
    else:
    ...
    ##################################################################################################################
    # step 2.5  generate train set and validation set given the setting files
    # generate train_file, val_file and test_file
    train_split_file, test_split_file = generate_train_test(in_dir=f'{root_dir}/datasets/refrigerator_frames',
                                                            video_type='_1.mp4',
                                                            test_size=0.3, random_state=42,
                                                            modality=modality, args=args)
    # get train and val
    train_split_file, val_split_file = generate_train_val(train_split_file,
    ...
    test_dataset = datasets.__dict__[args.dataset](root=dataset,
    ...
    test_loader = torch.utils.data.DataLoader(
        ...
        batch_size=args.batch_size, shuffle=False,
        num_workers=args.workers, pin_memory=True)
    print('{} samples found, {} train samples, {} val samples, and {} test samples.'.format(
    ...
    for epoch in range(startEpoch, args.epochs):
        ...
        train_loss = train(train_loader, model, criterion, criterion2, optimizer, epoch, modality)
        ...
        if (epoch + 1) % args.save_freq == 0:
            ...
            val_losses.append(lossClassification)
    ...
    # ----test
    print('\n\ntesting')
    prec1, prec3, lossClassification = validate(test_loader, model, criterion, criterion2, modality, val_type='test')
    print('finished!')
...
def parse_cmd():
    ...
    parser.add_argument('--settings', metavar='DIR', default=f'{root_dir}/datasets/settings',
                        help='path to datset setting files')
    # parser.add_argument('--modality', '-m', metavar='MODALITY', default='rgb',
    #                    choices=["rgb", "flow"],
    #                    help='modality: rgb | flow')
    parser.add_argument('--dataset', '-d', default='refrigerator',
    ...
    parser.add_argument('-j', '--workers', default=0, type=int, metavar='N',
                        help='number of data loading workers (default: 2)')  # once the j > 0, it stucks at train_loader?
