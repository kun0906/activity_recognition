...
def VideoSpatialPrediction3D(
    ...
    if len(offsetMainIndexes) == 0:
        offsets = list(range(1, duration + 2)) * int(np.floor(length / (duration + 1))) + list(
    ...
    for index in offsets:
        ...
        imageList1.append(
            img[int(16 * scale):int(16 * scale + imageSize), int(58 * scale): int(58 * scale + imageSize), :])
        ...
        imageList6.append(
            img_flip[int(16 * scale):int(16 * scale + imageSize), int(58 * scale): int(58 * scale + imageSize), :])
