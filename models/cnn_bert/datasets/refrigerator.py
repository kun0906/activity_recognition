    ...
    if not os.path.exists(source):
        ...
        with open(source) as split_f:
            ...
            for line in data:
                ...
                duration = len([1 for v in os.listdir(clip_path) if 'img' in v])
                target = int(line_info[2])
                item = (clip_path, duration, target)
                clips.append(item)
                classes.append(line_info[0])
    classes.sort()
    class_to_idx = {classes[i]: i for i in range(len(classes))}
    return clips, classes, class_to_idx
...
class refrigerator(data.Dataset):
    ...
    def __init__(self,
        ...
        clips, classes, class_to_idx = make_dataset(root, source)
